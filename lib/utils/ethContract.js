...
module.exports = class EthContract extends EthClass {
    ...
    async contractSend(contractAddress, method, privateKey) {
        ...
        //0xf3d603f26891740268db6838a5e97b168f9c22f323c0111b70b89261035c6cc6

    }

    //发布合约
    async delopy(contractByteCode, privateKey, gas, initArg) {
        let contarctSign = await this.sign(contractByteCode, privateKey, gas, initArg).catch(e => {
            throw e
        })
    ...
    async sign(byteCode, privateKey, gas, initArg) {
        let account = this.web3.eth.accounts.privateKeyToAccount(privateKey)
        let gasPrice = await this.web3.eth.getGasPrice()
        let nonce = await this.web3.eth.getTransactionCount(account.address)
        ...
        let tx = {
            gasPrice, gas, nonce, data, arguments: initArg
        }

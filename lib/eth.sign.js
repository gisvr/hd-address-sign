...
module.exports = class EthSign extends Sign {
    constructor(opts, url) {
        ...
        if (url) {
            ...
            // web3 代理 需要设置环境变量 NODE_TLS_REJECT_UNAUTHORIZED = 0
    ...
    signTx(rawTx, privateKey) {
        ...
        // web3.utils.sha3(string) //keccak256(string) // ALIAS

        let ll = ethTx.getSenderAddress()
        console.log(ll.toString("hex"))

        return {
            messageHash: '0x' + ethTx.hash(false).toString('hex'),
